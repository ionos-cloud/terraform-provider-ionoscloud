...
const (
	...
	Version               = "products/dbaas/mariadb/v3.0.0"
	DefaultIonosServerUrl = "https://mariadb.de-txl.ionos.com/v2"
	DefaultIonosBasePath  = "/v2"
)

// APIClient manages communication with the IONOS CLOUD DBaaS MariaDB API v2.0.0
...
func NewAPIClient(cfg *shared.Configuration) *APIClient {
	...
	*cfgCopy = *cfg
	...
	if cfgCopy.UserAgent == "" {
		cfgCopy.UserAgent = "sdk-go-bundle/products/dbaas/mariadb/v3.0.0"
	}
	...
	if len(cfgCopy.Servers) == 0 {
		cfgCopy.Servers = shared.ServerConfigurations{
			{
				URL:         "https://mariadb.de-txl.ionos.com/v2",
				Description: "Germany, Berlin",
			},
			{
				URL:         "https://mariadb.de-fra.ionos.com/v2",
				Description: "Germany, Frankfurt (de/fra, de/fra/2)",
			},
			{
				URL:         "https://mariadb.fr-par.ionos.com/v2",
				Description: "France, Paris",
			},
			{
				URL:         "https://mariadb.es-vit.ionos.com/v2",
				Description: "Spain, Logroño",
			},
			{
				URL:         "https://mariadb.gb-lhr.ionos.com/v2",
				Description: "United Kingdom, London",
			},
			{
				URL:         "https://mariadb.gb-bhx.ionos.com/v2",
				Description: "United Kingdom, Worcester",
			},
			{
				URL:         "https://mariadb.us-las.ionos.com/v2",
				Description: "United States, Las Vegas",
			},
			{
				URL:         "https://mariadb.us-mci.ionos.com/v2",
				Description: "United States, Lenexa",
			},
			{
				URL:         "https://mariadb.us-ewr.ionos.com/v2",
				Description: "United States, Newark",
			},
		}
		shared.LogDebug("[DEBUG] mariadb: using default endpoint: %s", cfgCopy.Servers[0].URL)
	} else {
		shared.LogDebug("[DEBUG] mariadb: server config provided with %d endpoint(s)", len(cfgCopy.Servers))
	...
	for i, srv := range cfgCopy.Servers {
		shared.LogDebug("[DEBUG] mariadb: final endpoint[%d]: url=%s description=%q", i, srv.URL, srv.Description)
	}
...
func (c *APIClient) callAPI(request *http.Request) (*http.Response, time.Duration, error) {
	...
	for {
		...
		if err != nil {
			shared.LogDebug("[DEBUG] mariadb: request failed for %s %s: %v", clonedRequest.Method, clonedRequest.URL.Host, err)
		...
		case http.StatusInternalServerError:
			...
			shared.LogDebug("[DEBUG] mariadb: received %d for %s %s, will retry (attempt %d/%d)", resp.StatusCode, request.Method, request.URL.String(), retryCount, c.GetConfig().MaxRetries)
			backoffTime = c.GetConfig().WaitTime
		case http.StatusServiceUnavailable,
			http.StatusGatewayTimeout,
			http.StatusBadGateway:
			if request.Method == http.MethodPost {
				shared.LogDebug("[DEBUG] mariadb: received %d for POST %s, not retrying (non-idempotent)", resp.StatusCode, request.URL.String())
				return resp, httpRequestTime, err
			}
			shared.LogDebug("[DEBUG] mariadb: received %d for %s %s, will retry (attempt %d/%d)", resp.StatusCode, request.Method, request.URL.String(), retryCount, c.GetConfig().MaxRetries)
			backoffTime = c.GetConfig().WaitTime
		...
		case http.StatusTooManyRequests:
			if retryAfterSeconds := resp.Header.Get("Retry-After"); retryAfterSeconds != "" {
				...
				shared.LogDebug("[DEBUG] mariadb: rate limited (429) for %s %s, retry-after=%ss (attempt %d/%d)", request.Method, request.URL.String(), retryAfterSeconds, retryCount, c.GetConfig().MaxRetries)
			} else {
				backoffTime = c.GetConfig().WaitTime
				shared.LogDebug("[DEBUG] mariadb: rate limited (429) for %s %s, using default backoff (attempt %d/%d)", request.Method, request.URL.String(), retryCount, c.GetConfig().MaxRetries)
			}
		default:
			return resp, httpRequestTime, err
		...
		if retryCount >= c.GetConfig().MaxRetries {
			shared.LogDebug("[DEBUG] mariadb: retry exhausted after %d attempts for %s %s, last status=%d", retryCount, request.Method, request.URL.String(), resp.StatusCode)
			break
		} else {
