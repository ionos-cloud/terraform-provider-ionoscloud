...
func TestAccServerBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccDataSourceServerPartialMatchName,
				Check: resource.ComposeTestCheckFunc(
			...
			{
				Config:      testAccDataSourceServerWrongPartialNameError,
				ExpectError: regexp.MustCompile(`no server found with the specified criteria: name`),
			},
			{
...
const testAccDataSourceServerPartialMatchName = testAccCheckServerConfigBasic + `
data ` + ServerResource + ` ` + ServerDataSourceByName + ` {
  datacenter_id = ` + DatacenterResource + `.` + DatacenterTestResource + `.id
...
const testAccDataSourceServerWrongPartialNameError = testAccCheckServerConfigBasic + `
data ` + ServerResource + ` ` + ServerDataSourceByName + ` {
  datacenter_id = ` + DatacenterResource + `.` + DatacenterTestResource + `.id
  name			= "wrong_name"
  partial_match	= true
