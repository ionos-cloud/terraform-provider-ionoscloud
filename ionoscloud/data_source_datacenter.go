...
func dataSourceDataCenterRead(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	if idOk {
		datacenter, apiResponse, err = client.DataCenterApi.DatacentersFindById(ctx, id.(string)).Execute()
		...
		datacenters, apiResponse, err := client.DataCenterApi.DatacentersGet(ctx).Depth(1).Execute()
		logApiRequestTime(apiResponse)
