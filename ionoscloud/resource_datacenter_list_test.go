...
func TestDatacenterListResource(t *testing.T) {
	...
	t.Run("streams every datacenter", func(t *testing.T) {
		results := listResults(ctx, t, server, datacenterListType, listSchema, nil, true)
		if len(results) != 2 {
	...
	t.Run("applies filters", func(t *testing.T) {
		results := listResults(ctx, t, server, datacenterListType, listSchema, map[string]string{"location": "de/fra"}, true)
		if len(results) != 1 {
...
func listResults(ctx context.Context, t *testing.T, server tfprotov6.ProviderServer, typeName string, schema *tfprotov6.Schema, filters map[string]string, includeResource bool) []tfprotov6.ListResourceResult {
	t.Helper()
	...
	for result := range stream.Results {
		...
		if includeResource && result.Resource == nil {
