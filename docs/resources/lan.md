...
> **Note:** `terraform plan -generate-config-out` writes this computed attribute as an empty `ip_failover {}` block. The empty block is harmless and can be removed from the generated configuration.
