...
layout: "ionoscloud"
page_title: "IONOS CLOUD: server"
sidebar_current: "docs-resource-server"
description: |-
  Creates and manages IONOS CLOUD Server objects.
---
...
- `confidential` - (Optional)[bool] If set, creates a Confidential Computing (SEV-SNP) VM from a confidential boot image. Requires `type = "ENTERPRISE"` and a private SEV-SNP `image_name`. `cores` and `cpu_family` must not be set - both are derived from the image. This property is immutable. Only available in confidential-computing enabled locations.
