...
- `replica_configuration` - (Required)[List]  
    ...
    - `cpu_family` - (Required)[string] CPU family for the VMs created using this configuration. If null, the VM will be created with the default CPU family for the assigned location. Possible values are: `INTEL_SKYLAKE`, `INTEL_XEON`.
