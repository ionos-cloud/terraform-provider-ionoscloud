...
- `page_title` — `"IONOS CLOUD: <resource>"`, the **bare type stem**, no `ionoscloud_` prefix.
  All six existing list pages use this form. Do **not** copy the resource page's title: about a
...
two-filter example, and the `## Identity Attributes` section. Follow the newer form
(`datacenter.md`, `pg_cluster_v2.md`) instead.
