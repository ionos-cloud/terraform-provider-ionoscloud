...
Eight checks, from four workflows. **There is no test job.** `test.yml` and every
`*-test-run.yml` are `workflow_dispatch`/`schedule` only, so `go test` never runs
automatically. Green CI does not mean your tests pass — that is on you.

| Check | Workflow | Command |
|---|---|---|
| `Run gofmt check` | Sonarcloud and gofmt | `gofmt -l -d` over `.` |
| `Run go vet with all build tags` | Sonarcloud and gofmt | `go vet -tags=all ./...` |
...
| `lint` | CI golangci-lint | `golangci-lint run --timeout 10m0s --verbose --new-from-rev=origin/master` (v2.10.1) |
...
2. **`tagalign`** (`sort: true`, order `json, tfsdk, mapstructure`) — model struct tags must be
   sorted and aligned. `` `tfsdk:"id" json:"id"` `` fails.
3. **`revive`**'s configured `import-alias-naming` with `allowRegex: "^[a-z][a-z0-9]*$"` — an
   import alias may contain **no underscore and no uppercase letter**. `ionoscloudsdk` and
   `fwprovider` pass; `fw_provider` and `sdkV2` fail.
4. **`goconst`** (min-len 3, min-occurrences 5) — hence the package-level type-name const in
   the test.
5. **`prealloc`** — `make([]T, 0, len(src))`, not `var xs []T` before a range loop.
6. **`gocyclo`** (min-complexity 25) — live in the resource file. A big schema's mapper creeps
   past it; split the model fill into a helper rather than growing one method.
...
9. **Formatters** `gofmt` + `goimports` with
   ...
   `golangci-lint fmt ./internal/framework/services/<service>/... ./ionoscloud/...`.
...
> No datacenter read path in the provider paginates today — `ionoscloud/data_source_datacenter.go:141`
...
(`//go:build compute || all || <resource>`) and does not run on a PR. **Never fire it
unprompted** — it runs against real IONOS credentials and creates live resources.

What fires it: `.github/workflows/pr-comment-trigger.yaml:12-38` — a **top-level** PR comment
(`issue_comment`) from an OWNER/MEMBER/COLLABORATOR/CONTRIBUTOR whose *trimmed body begins
with* `/test <tag>` dispatches `e2e.yaml` (`TF_ACC: true`, `-timeout 6h`, real credentials).
Inline review replies use a different event (`pull_request_review_comment`) and cannot trigger
it. So: never start a top-level comment with `/test` — if you need to mention the command,
put it mid-sentence.
...
- **`ProtoV6ProviderFactories`, not `ProviderFactories`.** The list resource lives on the
  framework half of the mux; the ten `ionoscloud/` tests using `testAccProviderFactories`
  cannot see it. Use `testAccProtoV6ProviderFactoriesInternal(t, &testAccProvider)`
  (`ionoscloud/provider_test.go:103`).
- **`Query: true` is load-bearing on every list step.** It is what routes the step to
  `testStepNewQuery` (`vendor/.../helper/resource/testing_new.go:370`), what writes the config
...
(`vendor/.../helper/schema/provider.go:220-225`). Nothing else reaches it: no CI job runs any
test, rung 1 compiles no test file, and rung 2's `GetResourceIdentitySchemas` assertion passes
for a malformed identity. `go test ./ionoscloud/ -run 'TestProvider$' -count=1`.
