...
func (c *Client) ChangeConfigURL(ctx context.Context, location string) {
	...
	if location == "" && os.Getenv(ionosAPIURLLogging) != "" {
		...
		tflog.Debug(ctx, "Logging: endpoint from env", map[string]any{"env": ionosAPIURLLogging, "url": url})
	...
	for _, server := range config.Servers {
		if strings.EqualFold(server.Description, shared.EndpointOverridden+location) || strings.EqualFold(server.URL, locationToURL[location]) {
			tflog.Debug(ctx, "Logging: endpoint for location", map[string]any{"location": configlog.FormatLocation(location), "url": server.URL})
	...
	tflog.Debug(ctx, "Logging: endpoint for location", map[string]any{"location": configlog.FormatLocation(location), "url": locationToURL[location]})
}

var (
	ionosAPIURLLogging = "IONOS_API_URL_LOGGING"

	AvailableLocations = []string{"de/fra", "de/fra/1", "de/fra/2", "de/txl", "es/vit", "gb/lhr", "fr/par"}
	...
	locationToURL   = map[string]string{
		...
		"de/fra/1": "https://logging.de-fra.ionos.com",
