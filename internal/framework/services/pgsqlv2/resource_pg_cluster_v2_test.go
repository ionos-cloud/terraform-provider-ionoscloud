...
func TestAccPgClusterV2(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Query: true,
				Config: `list "ionoscloud_pg_cluster_v2" "test" {
  provider = ionoscloud
}`,
  ...
  config {
    filters = [
      ...
      { field_name = "location", field_value = "` + testLocation + `" },
			...
			// Filter by name + wrong location: must return 0, proving location is evaluated.
			{
				Query: true,
				Config: `list "ionoscloud_pg_cluster_v2" "test" {
  provider = ionoscloud
      ...
      { field_name = "location", field_value = "` + testLocationChanged + `" },
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(clustersDSAddr, "clusters.0.backup.location", clusterResourceAddr, "backup.location"),
