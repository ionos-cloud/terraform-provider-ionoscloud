//go:build all || objectstorage || userobjectstorage
// +build all objectstorage userobjectstorage
...
func TestAccBucketResource(t *testing.T) {
	...
	name := "ionoscloud_user_bucket.test"
...
func TestAccBucketResource_ForceDestroy(t *testing.T) {
	rName := acctest.GenerateRandomResourceName(bucketPrefix)
	name := "ionoscloud_user_object_storage_bucket.test"
...
resource "ionoscloud_s3_bucket_versioning" "bucket" {
  bucket = ionoscloud_user_object_storage_bucket.test.name
  versioning_configuration {
    status = "Enabled"
  }
}
