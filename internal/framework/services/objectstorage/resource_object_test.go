	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore:              []string{"force_destroy", "content", "server_side_encryption"},
...
func buildGetObjectRequest(ctx context.Context, client *objstorage.APIClient, attributes map[string]string) objstorage.ApiGetObjectRequest {
...
func testAccCheckObjectBody(got *string, want string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if *got != want {
			return fmt.Errorf("object body = %v, want %v", got, want)
		}
