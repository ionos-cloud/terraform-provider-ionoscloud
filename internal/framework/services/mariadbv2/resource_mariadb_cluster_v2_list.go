...
type clusterWithLocation struct {
	Cluster  mariadbv3.ClusterRead
	Location string
}

// NewClusterListResource creates a new list resource for mariadb_cluster_v2.
...
func (r *clusterResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	identity.StreamList(ctx, stream, req,
		func(ctx context.Context) ([]clusterWithLocation, error) {
			...
			locations := mariadbv2service.AvailableLocations()
			...
			for _, loc := range locations {
				client, err := r.bundle.NewMariaDBV2Client(ctx, loc)
				if err != nil {
					return nil, fmt.Errorf("failed to create MariaDB V2 client, location: %s, error: %w", loc, err)
				}
				result, _, err := client.ListClusters(ctx, nameFilter)
				if err != nil {
					return nil, fmt.Errorf("failed to list MariaDB V2 clusters, location: %s, error: %w", loc, err)
				}
...
func (r *clusterResource) mapCluster(ctx context.Context, includeResource bool, filters []identity.Filter, item clusterWithLocation) (*identity.MappedItem, diag.Diagnostics) {
	...
	mapClusterResponseToModel(ctx, &c, model)
