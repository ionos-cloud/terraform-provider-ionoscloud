...
func versionDataSourceAttributes() map[string]schema.Attribute {
	return map[string]schema.Attribute{
		...
		"comment":        schema.StringAttribute{Computed: true, Description: "Additional information about the version lifecycle."},
		...
		"version":        schema.StringAttribute{Computed: true, Description: "The MariaDB version string."},
	}
}
...
// NewVersionsDataSource creates a new data source for listing MariaDB v2 versions.
...
func (d *versionsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_mariadb_versions_v2"
}
...
func (d *versionsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Lists all supported IONOS CLOUD MariaDB V2 versions in a given location.",
...
func (d *versionsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	list, apiResponse, err := client.ListVersions(ctx)
	if err != nil {
		resp.Diagnostics.AddError("failed to list MariaDB V2 versions", diagutil.WrapError(err, &diagutil.ErrorContext{
...
func mapVersionToModel(ctx context.Context, props *mariadbv3.MariadbVersion, model *versionDataSourceModel) diag.Diagnostics {
	...
	upgrades := make([]types.String, 0, len(props.CanUpgradeTo))
	for _, v := range props.CanUpgradeTo {
		upgrades = append(upgrades, types.StringValue(v))
	}
